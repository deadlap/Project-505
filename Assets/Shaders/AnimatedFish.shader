...
/*Has been slightly modified in order to affect the correct parts of the texture, by changing 
certain y-axis to the z-axis*/
Shader "BitShiftProgrammer/AnimatedFish"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				v2f o;
				half sinUse = sin(_Time.z * _WaveSpeed + _MoveOffset + v.vertex.z * _WaveDensity);
				half zValue = v.vertex.z - _Zoffset;
				half zDirScaling = clamp(1.0,0.0,pow(zValue * _EffectRadius,_Threshold));
				v.vertex.x = v.vertex.x + sinUse * _WaveHeight* zDirScaling;
				v.vertex.x = v.vertex.x + sin(_Time.z * _StrideSpeed + _MoveOffset) * _StrideStrength;
